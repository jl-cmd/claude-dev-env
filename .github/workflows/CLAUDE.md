...
|------|-----------|--------------|
| `ci-tests.yml` | Push to `main`, PR against `main` | Cheap `changes` job (`dorny/paths-filter`) sets `python` / `javascript` outputs. On PRs, the Python suite (root `tests/` + `packages/claude-dev-env`, Python 3.12, deselect lists under `.github/ci/`) and JS suite (`npm test` in `packages/claude-dev-env`, Node 24) run only when their filter matches; push to `main` always runs both. `permissions.contents: read` only. |
