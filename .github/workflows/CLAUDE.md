...
|------|-----------|--------------|
| `ci-tests.yml` | Push to `main`, PR against `main` | Cheap `changes` job (`dorny/paths-filter`) sets `python` / `javascript` outputs. On PRs, the Python suite (root `tests/` + `packages/claude-dev-env`, Python 3.12, deselect lists under `.github/ci/`) and JS suite (`npm test` in `packages/claude-dev-env`, Node 24) run only when their filter matches; push to `main` always runs both. Always-on: quality-gate — full `packages/claude-dev-env/scripts/check.ps1` under pwsh (ruff + mypy + enforcer pytest, no Skip flags) with `PYTEST_ADDOPTS` from the four deselect lists (CI gate = script + tools + documented deselect set; local may run more) — and windows-semantics micro-suite on `windows-latest` selecting the seven node IDs from `.github/ci/windows-semantics-node-ids.txt`. Permissions: `contents: read` and `pull-requests: read` (paths-filter on PRs). |
...
- `ci-tests.yml` uses `actions/checkout@v5`, `dorny/paths-filter` (SHA-pinned), `actions/setup-python@v5` (Python 3.12), and `actions/setup-node@v4` (Node 24). Path filters skip inert PR paths; push to `main` always runs full suites. v1 includes all of `packages/claude-dev-env/**` in both filters (package markdown is fixture-bearing). Deselect node IDs live in `.github/ci/live-post-audit-deselects.txt`, `.github/ci/windows-semantics-node-ids.txt`, `.github/ci/known-pending-deselects.txt`, and `.github/ci/author-swap-deselects.txt`. The why for each family is the local-only register in `tests/CLAUDE.md`. The root suite covers the sync-ai-rules modules whenever the python filter matches, so there is no separate path-filter workflow for those two test files. The quality-gate job reuses those lists as `PYTEST_ADDOPTS --deselect=` values (full repo-relative node IDs; no path rewriting) and runs the full `check.ps1` gate without `-SkipRuff`/`-SkipMypy`. The windows-semantics job reads only `windows-semantics-node-ids.txt` and passes those node IDs as a positional select list.
