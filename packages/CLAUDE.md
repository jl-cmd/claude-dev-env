...
`packages/claude-dev-env` is the install target users reach via `npx claude-dev-env`. The monorepo root hosts shared config (`.codemaprc`, `audits/`) that is not installed into `~/.claude/`.
