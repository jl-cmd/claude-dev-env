...
The mandate covers the shell a command runs through, not every executable a command names. A direct interpreter invocation another rule documents — the paramiko NAS helper in `nas-ssh-invocation.md`, a `python` call on a repo script — conforms as written.
