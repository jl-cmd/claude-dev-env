...
| `few-words.md` | small words. few words. always. forever. — also syncs to Cursor as an always-on `.mdc` rule, since Cursor's `beforeSubmitPrompt` hook cannot inject context |
