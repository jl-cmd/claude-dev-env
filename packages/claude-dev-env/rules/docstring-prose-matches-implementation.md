...
- **Gate-outcome status flags** — an outcome the code routes to a blocking `break` reads as a blocker on every surface (schema, architecture doc, reference `.md`), never as a bypass.
