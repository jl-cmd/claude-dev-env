...
- **Hand-typed pytest counts on create/edit.** On `gh pr create` / `gh pr edit`, a body that both mentions `pytest` and carries a prose `N passed` / `N failed` claim is denied. Put measured counts in a fenced block (exempt) or in a `gh pr comment` proof post — not free prose in the description.
