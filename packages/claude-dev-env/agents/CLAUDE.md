...
| `code-verifier.md` | Code Verifier | Post-hoc verification after coder agents finish; read-only in the verified tree apart from a deliberate break in a scratch copy outside it, fresh context, puts the draft verdict through one strongest-tier validation subagent, ends with a fenced verdict |
