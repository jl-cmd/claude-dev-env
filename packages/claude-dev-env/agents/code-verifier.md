...
Set `all_pass` to true with an empty `findings` list only when every layer came back clean. Always include `manifest_sha256` so the verdict clears the commit regardless of which work tree the verifier or the committer ran in. The spawning agent stages the final repair and runs `code_rules_gate.py --staged`; that gate stores a staged-index attestation bound to the worktree, HEAD, and index tree. This verifier starts after the spawn gate confirms that matching attestation. Commit-committability gates (CODE_RULES / merge conflicts) are green before you are spawned; you are the final semantic check before commit. Any file change after you finish moves that hash and invalidates the verdict.
