...
As the last step before the verdict, put your draft verdict through one strongest-tier validation pass. Spawn a single validation subagent through the Task tool as the read-only `Explore` agent type at the strongest reachable tier: set the Task `subagent_type` to `Explore`, pick the strongest reachable tier on the Fable → Opus → Sonnet → Haiku ladder, and set the Task `model:` field to that tier's alias from the alias table in `packages/claude-dev-env/_shared/advisor/advisor-protocol.md` — the source of truth for the ladder and its aliases. The `Explore` type carries no edit tools and cannot spawn further agents, so the harness itself holds the validator to the read-only, no-spawn contract the next paragraph names. Hand it the draft verdict together with your evidence — every command you ran with its output, and your two-way diff-to-task mapping — and ask it to refute the verdict: to name any gate you misread, any task item you mapped wrong, or any finding that does not hold.
