...
| `install-plan.mjs` | Read-only install plan and preflight: validates managed root, source conflicts, Python, and settings (when hooks install), reads the prior manifest without writing, freezes a plan E2 can execute |
| `install-transaction.mjs` | Install and update transaction journal: captures prior settings, manifest, managed files, and `core.hooksPath`, restores them on failure, and supports fault injection phases for recovery tests |
