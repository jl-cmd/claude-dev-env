...
| `install-constants.mjs` | The named values `install.mjs` reads: `SKIPPED_SOURCE_ENTRY_NAMES` and `SKIPPED_SOURCE_FILE_EXTENSIONS` for the build artifacts the source walk leaves behind, `RUN_BACKUP_DIRECTORY_NAME_PATTERN` for the timestamp shape a run backup directory carries, and `MANAGED_SKILLS_DIRECTORY_NAME`, `MANAGED_HOOKS_DIRECTORY_NAME`, and `SETTINGS_FILE_NAME` for the `~/.claude` paths the copy loops and the prunes share |
...
A run writes both manifest keys — the file list and the skill-name list — wholesale from what it just installed only when the prunes ran that run, so the next diff reads as "the package stopped shipping this". Every other run unions what it wrote onto the prior lists: a scoped `--only` install, and a full install holding its prunes behind an unresolved dependency group. The union keeps every entry a later prune needs to spot a stale file or a retired skill, and keeps `--uninstall` able to name the whole tree. The prune itself bounds the lists: a stale path leaves the record on the first full install that moves it aside.
...
Each record passes a containment guard first: the path resolves under `~/.claude`, or the purge skips it with a warning and counts it. Skipping keeps one malformed record — hand-edited, or written by an installer that ran against a different home — from stranding the user with a half-removed install. The purge removes every legitimate record, clears the manifest, and reports the skipped count.
...
| `MANAGED_TOP_LEVEL_DIRECTORY_NAMES` | The content directories plus `skills` and `hooks`; the uninstall purge reads it to find the root a recorded file belongs to |
