...
test('a reinstall removes a retired post-commit shim and preserves a foreign git hook', () => {
    const sandbox = createSandbox();
    try {
        runInstaller(sandbox.homeDirectory, []);
        ...
        writeFileWithParents(postCommitShimPath, 'generated by an earlier install\n');
        ...
        assert.equal(existsSync(postCommitShimPath), false);
        assert.deepEqual(readFileSync(foreignHookPath), foreignHookContentBytes);
        assert.equal(freshManifest.files.includes(postCommitShimPath), false);
        assert.equal(freshManifest.files.includes(foreignHookPath), false);
        for (const gitHookName of [PRE_COMMIT_HOOK_NAME, PRE_PUSH_HOOK_NAME]) {
