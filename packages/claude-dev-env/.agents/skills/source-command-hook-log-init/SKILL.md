---
name: "source-command-hook-log-init"
description: "Initialize the Neon schema used by the hook-log extractor"
---

# source-command-hook-log-init

Use this skill when the user asks to run the migrated source command `hook-log-init`.
...
4. **Install the Python dependencies** so the extractor can reach Neon.
   ...
   ```
   pip install -r packages/Codex-dev-env/hooks/diagnostic/requirements-hook-logs.txt
   ```

   Development (adds `pytest` on top of the runtime deps):

   ```
   pip install -r packages/Codex-dev-env/hooks/diagnostic/requirements-hook-logs-dev.txt
   ```

## Run the init

```
bws run -- python packages/Codex-dev-env/hooks/diagnostic/hook_log_init.py
```
...
3. Applies the DDL in `packages/Codex-dev-env/hooks/diagnostic/schema.sql`
