---
name: pr-name-by-capability-audit
description: >-
  ...
  /pr-name-by-capability-audit, name-by-capability audit, audit PR for naming, name
...
1. `preflight-proposal` runs `pr-name-by-capability-audit preflight-proposal <pr_number> --base-sha <base_sha> --head-sha <head_sha> --worktree <isolated_worktree>` under the [shared preflight proposal contract](../_shared/pr-loop/preflight-proposal.md). Record each finding with violation or OK-driver classification.
...
- User invokes `/pr-name-by-capability-audit <PR>` or asks to audit a PR for name-by-capability / capability naming.
...
| Peer | Relationship |
|------|----------------|
| `pr-shared-extraction-audit` | Layering and extraction (where code lives). Invoke when a path’s *role* is unclear; this skill only scores the *name*. |
