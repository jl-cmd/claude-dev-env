---
name: "source-command-hook-log-extract"
description: "Extract hook-firing records from session transcripts into Neon and show blocker summary"
---

# source-command-hook-log-extract

Use this skill when the user asks to run the migrated source command `hook-log-extract`.

## Command Template

Scan every JSONL session transcript under `~/.Codex/projects/` (or the
...
```
bws run -- python packages/Codex-dev-env/hooks/diagnostic/hook_log_extractor.py
```

Full extraction using the current byte offsets in
`~/.Codex/logs/hooks/.state/offsets.json` (override the `~/.Codex`
...
```
bws run -- python packages/Codex-dev-env/hooks/diagnostic/hook_log_extractor.py --full-rebuild
```
...
```
bws run -- python packages/Codex-dev-env/hooks/diagnostic/hook_log_extractor.py --summary
```
...
```
bws run -- python packages/Codex-dev-env/hooks/diagnostic/hook_log_extractor.py --query <name>
```
...
`packages/Codex-dev-env/hooks/diagnostic/queries/`:
...
ISO-8601 line to `~/.Codex/logs/hook-extractor.log` (override the
`~/.Codex` root with the `CLAUDE_HOME` env var) and exits with
