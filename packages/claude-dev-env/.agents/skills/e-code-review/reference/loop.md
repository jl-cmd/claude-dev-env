...
A round always applies fixes. Gate 2 below is the only place a round applies a
fix. `--fix` chooses the mechanics. With the flag, Gate 2 loads
`reference\fix.md` (relative to this skill's folder) for which agent applies
each fix, agent resume, the code-rules gate, skip logging, and outcome
reporting. Without the flag, this session applies each fix itself.

The gate sequence still decides whether a round fixes, commits, pushes, and
re-reviews.
...
`~/.claude/scripts/code_rules_gate.py --repo-root <repo root>`
