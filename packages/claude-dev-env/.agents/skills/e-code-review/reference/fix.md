...
1. Run `~/.claude/scripts/code_rules_gate.py --repo-root <repo root>` with no
   file paths and no `--only-under` prefix.
...
downstream commits either. A one-shot fix pass leaves its fixes uncommitted in
the working tree for the lead to review and commit (stage first if you re-run
the gate on those lines). Under `loop`, the round tail in `reference\loop.md`
stages and runs required checks; the lead still owns the commit and push.
...
list presented to the user at the end of the fix pass, after every other
finding has been fixed, marked not-needed, or skipped, and never before.
...
Whenever a reported finding is fixed later in the session, because the user
asks for the fix or later work fixes it incidentally, call the structured
