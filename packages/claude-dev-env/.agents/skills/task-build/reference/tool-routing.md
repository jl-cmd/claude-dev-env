...
4. **File ledger fallback.** During an orchestrator run with no host task or plan tool, register work in the durable `grok_run_ledger` under the run-state directory so orchestration stays reconcilable across hosts.
