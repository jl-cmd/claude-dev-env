...
a deterministic static sweep, then the built-in `/code-review ultra --fix` on the
...
b. Run the built-in `/code-review ultra --fix` with OPUS on the FULL `origin/main...HEAD`
   ...
   - **`mode == "in_session"`** (Claude host and session model is opus): run
     `/code-review ultra --fix` with OPUS in this session with no path arguments so it
   ...
   - **`mode == "chain"`** (any other host, or a Claude session on any model
     ...
     (`claude -p "/code-review ultra --fix" --model opus` through the chain
