...
| grant_project_claude_permissions.py | Writes idempotent allow-rules and additionalDirectories into ~/.claude/settings.json |
| revoke_project_claude_permissions.py | Removes the allow-rules and entries that the grant script wrote |
...
| terminology_sweep.py | Flags a prose term that near-misses an identifier on added code lines of a unified diff |
| reviews_disabled.py | Shared helper for the reviewer opt-out and opt-in gates |
| copilot_quota.py | Copilot premium-request quota pre-check |
| reviewer_availability.py | Unified reviewer-availability entry point for Copilot and Bugbot |
...
| build_converge_task_list.py | Step-1 task list: runnable review gates plus final clean-same-head gate |
...
| tests/ | pytest suite for the gate/permission/reviewer scripts |
