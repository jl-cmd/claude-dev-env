...
Canonical: `~/.claude/skills/_shared/pr-loop/scripts/code_rules_gate.py` after install.
...
Cross-skill path traversal works because every skill installs as a flat top-level dir under `~/.claude/skills/` and `_shared/` installs alongside those skills in that same directory.
