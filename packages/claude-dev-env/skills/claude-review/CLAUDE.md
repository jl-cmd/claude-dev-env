...
drained). On clean pass, posts a best-effort PR issue comment. Triggered by
`/claude-review`, `claude-review`, `xhigh`, `code-review xhigh --fix`, thorough
review on opus, clean comment, or colloquial ultra-review.
...
stay in shared scripts / `pr-fix-protocol`. Clean comment is soft-fail and
idempotent per HEAD. Converge loops own phase stamps (`code_review_clean_at`)
and re-entry. Not the workflow `converge.mjs` code-review lens.

## Key files

| File | Purpose |
|---|---|
| `SKILL.md` | When, refusals, composition, process, probe + invoker CLI, gotchas |
| `reference/full-diff-and-clean-stamp.md` | Full-diff rule, clean-stamp, clean-comment contract |
...
| `../../scripts/post_claude_review_clean_comment.py` | Best-effort PR issue comment after a clean pass (soft-fail, idempotent per HEAD) |
