...
10. Only when the operator passed `--split-further`: run the split-further loop ([split-further-loop.md](split-further-loop.md)) — BFS re-split each draft at `--recursion-depth 1` until stop reasons drain the queue. Otherwise stop after step 9.
