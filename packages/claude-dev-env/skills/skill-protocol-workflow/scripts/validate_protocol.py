...
def _resolve_schema_reference(all_schema_node: Mapping[str, object], all_schema_root: Mapping[str, object]) -> Mapping[str, object]:
    """Resolve a local `$defs` schema reference."""
    schema_reference = all_schema_node.get("$ref")
    if schema_reference is None:
        return all_schema_node
    if not isinstance(schema_reference, str) or not schema_reference.startswith("#/$defs/"):
    ...
    if not isinstance(definition, dict):
        raise ProtocolValidationError(f"record schema definition is missing: {definition_name}")
    return definition


def _validate_schema_node(candidate: object, all_schema_node: Mapping[str, object], all_schema_root: Mapping[str, object], field_path: str) -> None:
    ...
    if "const" in resolved_schema and candidate != resolved_schema["const"]:
        raise ProtocolValidationError(f"{field_path} does not match its schema constant")
...
def _validate_schema_object(candidate: object, all_schema_node: Mapping[str, object], all_schema_root: Mapping[str, object], field_path: str) -> None:
    ...
    if all_schema_node.get("additionalProperties") is False:
        additional_fields = sorted(each_field_name for each_field_name in candidate if each_field_name not in properties)
        if additional_fields:
            raise ProtocolValidationError(f"{field_path} has additional fields: {MISSING_FIELD_SEPARATOR.join(additional_fields)}")
    for each_field_name, each_schema_node in properties.items():
        if each_field_name in candidate and isinstance(each_schema_node, dict):
            _validate_schema_node(candidate[each_field_name], each_schema_node, all_schema_root, f"{field_path}.{each_field_name}")


def _validate_schema_array(candidate: object, all_schema_node: Mapping[str, object], all_schema_root: Mapping[str, object], field_path: str) -> None:
    ...
    if isinstance(minimum_array_entries, int) and len(candidate) < minimum_array_entries:
        raise ProtocolValidationError(f"{field_path} must contain at least {minimum_array_entries} items")
    ...
    if isinstance(array_entry_schema, dict):
        for each_index, each_candidate in enumerate(candidate):
            _validate_schema_node(each_candidate, array_entry_schema, all_schema_root, f"{field_path}[{each_index}]")


def _validate_schema_string(candidate: object, all_schema_node: Mapping[str, object], field_path: str) -> None:
...
def _load_json_object(task_run_path: Path) -> Mapping[str, object]:
    ...
    try:
        ...
        raise ProtocolValidationError(f"cannot read task-run record: {error}") from error
...
def _require_fields(all_task_run_record: Mapping[str, object], all_required_field_names: Sequence[str], record_name: str) -> None:
    ...
    """
    missing_field_names = [each_field_name for each_field_name in all_required_field_names if each_field_name not in all_task_run_record]
    if missing_field_names:
        raise ProtocolValidationError(f"{record_name} missing: {MISSING_FIELD_SEPARATOR.join(missing_field_names)}")
...
def _require_text(all_task_run_record: Mapping[str, object], field_name: str, record_name: str) -> None:
    ...
    if not isinstance(field_text, str) or not field_text:
        raise ProtocolValidationError(f"{record_name}.{field_name} must be a non-empty string")


def _require_string_list(all_task_run_record: Mapping[str, object], field_name: str, record_name: str) -> None:
    ...
    if not isinstance(all_field_entries, list) or any(
        not isinstance(each_entry, str) or not each_entry for each_entry in all_field_entries
    ):
        raise ProtocolValidationError(f"{record_name}.{field_name} must be a string list")
...
def _require_record(all_task_run_record: Mapping[str, object], field_name: str) -> Mapping[str, object]:
...
def _validate_review_record(all_review_record: Mapping[str, object]) -> None:
    ...
    _require_string_list(all_review_record, "findings", "review_record")
...
def _validate_verification_record(all_verification_record: Mapping[str, object], record_name: str) -> None:
    ...
    """
    _reject_additional_fields(all_verification_record, ALL_VERIFICATION_FIELDS, record_name)
...
def validate_record(all_task_run_record: Mapping[str, object], schema_path: Path) -> None:
    ...
    _reject_additional_fields(all_task_run_record, ALL_RECORD_FIELDS, "record")
    ...
    if not isinstance(commit_hash, str) or not MINIMUM_COMMIT_HASH_LENGTH <= len(commit_hash) <= MAXIMUM_COMMIT_HASH_LENGTH or COMMIT_HASH_PATTERN.fullmatch(commit_hash) is None:
        raise ProtocolValidationError(COMMIT_HASH_ERROR_TEMPLATE % (MINIMUM_COMMIT_HASH_LENGTH, MAXIMUM_COMMIT_HASH_LENGTH))
    ...
    _validate_verification_record(_require_record(all_task_run_record, "reverification_record"), "reverification_record")
    _validate_verification_record(_require_record(all_task_run_record, "verification_record"), "verification_record")
...
def main(all_cli_arguments: Sequence[str]) -> int:
    ...
    """
    if len(all_cli_arguments) != ARGUMENT_COUNT_REQUIRED:
        print("usage: validate_protocol.py <record.json>", file=sys.stderr)
        return EXIT_CODE_INVALID_RECORD
    schema_path = Path(__file__).parent.parent / "reference" / RECORD_SCHEMA_FILENAME
    try:
        validate_record(_load_json_object(Path(all_cli_arguments[1])), schema_path)
