# Self-Audit Task Seeds

Register each item as a host task before delivery. Complete each with PASS,
FAIL plus file and line evidence, or N/A plus a reason.

1. Confirm the hub has Contents, When this applies, Gotchas, File index, and Folder map sections.
2. Confirm every companion reference is linked directly from the hub.
3. Confirm the ordered task-seed catalog and task-tool seeding instruction are present.
4. Confirm every process step is classified with a home and evidence path.
5. Confirm final-validation and self-audit seed lists are present.
6. Confirm concrete review-loop examples show findings, repair, reverify, amend, and clean review.
7. Confirm the schema requires task identity, commit, review, repair, reverification, and verification records.
8. Confirm no unrelated route strings or absolute machine paths appear.
9. Confirm the focused contract tests pass and the owned-file diff is complete.
