...
const HEADLESS_SAFETY_PREAMBLE =
  ...
  '- rm shape rules — the hook auto-allows rm only when ALL of these hold: (1) standalone Bash call (never chained after cd or joined with && / || / ; / |); (2) absolute, literal path as the target — any $ in the target (variable, brace expansion, or command substitution) makes the gate fail closed; (3) target resolves inside /tmp, /temp, or the OS temp root. $CLAUDE_JOB_DIR/tmp is NOT auto-allowed because it expands via a $ variable. For any cleanup whose path is variable-built or whose setup/teardown spans multiple steps, author a Python helper file and run it as python <file>.py — that keeps every destructive literal out of a Bash command string entirely.\n' +
