  ...
  if (task === 'post-clean-audit') {
    ...
    return convergeReadOnlyAgent(
      `Transcribe a completed audit result to the PR thread for ${prCoordinates} at commit ${context.head}. You are relaying results that ${ranCount} audit lens(es) already produced on this HEAD earlier in this run — you are not judging the code yourself or clearing a merge gate.\n\n` +
        ...
        `Run the script with --help first if any flag name differs. This posts the CLEAN audit review body that check_convergence.py reads for the bugteam gate; the review event is APPROVE, or COMMENT when the author reviews their own PR with no alternate reviewer account. Do not edit code, commit, or push.\n\n` +
        `Read the script's stdout as a closed set of three cases. When line 1 is a review URL and there is no second line, return {posted:true, reviewUrl:<that URL>, reason:""}. When line 1 is a review URL and line 2 is the self-approval downgrade marker, return {posted:true, reviewUrl:<that URL>, reason:""} — the marker means the review posted as a COMMENT because the author has no alternate reviewer account. In any other case (a denial such as a permission prompt or auto-mode-classifier block, an error, or output whose first line is not a review URL), return {posted:false, reviewUrl:"", reason:<the denial message or error as one line>}. Do not retry a denied post.`,
