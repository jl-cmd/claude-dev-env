...
const HEADLESS_SAFETY_PREAMBLE =
  ...
  'WAITS AND POLLS — foreground sleep is blocked in this headless harness: a bare Bash "sleep N" or a PowerShell "Start-Sleep" is denied, and a wait you move to a background process and then end your turn to await never resumes, because a schema-bearing agent runs for a single turn. Therefore:\n' +
  '- Perform every required delay or poll-interval wait inside this same turn with the Monitor tool: arm it to re-check the step condition on the interval the step names, up to the attempt budget the step names, and let it return to you when the condition holds or the budget is spent. Never move a wait to a background process, and never end your turn to wait for something to finish.\n' +
  '- When your run was given a result schema, your final action is always the StructuredOutput call. If the poll budget is spent before the awaited signal arrives, call StructuredOutput with the time-out result the step documents (for the Copilot gate, down: true) rather than ending the turn without a result.\n\n'
