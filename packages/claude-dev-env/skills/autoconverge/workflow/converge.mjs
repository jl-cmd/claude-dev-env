...
const HEADLESS_SAFETY_PREAMBLE =
  ...
  '- rm shape rules — the hook grants several rm auto-allow paths; the simplest one accepts a standalone Bash call whose target resolves inside the ephemeral namespace (/tmp, /temp, the OS temp root, or the run worktree), and a compound path accepts an rm chained after a cd or joined with benign segments when every rm target is an absolute ephemeral path. Across these paths $(...) command substitution and backtick subshells always fail closed, and an unknown variable in the target fails closed; the known temporary variables TEMP, TMP, TMPDIR, and CLAUDE_JOB_DIR resolve to the OS temp root instead, so a bare $CLAUDE_JOB_DIR/tmp/<name> target is auto-allowed under an ephemeral cwd. Even so, prefer a Python helper for any cleanup whose path is variable-built or whose setup/teardown spans multiple steps: author the helper file and run it as python <file>.py, which keeps every destructive literal out of a Bash command string entirely and never depends on which auto-allow path matches.\n' +
