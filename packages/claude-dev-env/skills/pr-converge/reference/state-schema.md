...
- `bugbot_down`: boolean, init `false`. Set `true` when bugbot fails to
  ...
  at every BUGBOT-phase entry whenever the availability gate reports Bugbot
  disabled for the run — the default unless `CLAUDE_REVIEWS_ENABLED` lists
  the `bugbot` token, and always when `CLAUDE_REVIEWS_DISABLED` lists it
  (the BUGBOT entry gate in `per-tick.md`), which routes straight to BUGTEAM
  before any bugbot fetch or trigger. Also set `true` when an acknowledged
  trigger has been outstanding more than 30 minutes with no surfaced review
  at `current_head` (per Step 2 BUGBOT (c) 30-minute budget — see
  `per-tick.md`). Reset to `false` on every push; the entry gate re-applies
  the availability check on the next BUGBOT entry.
