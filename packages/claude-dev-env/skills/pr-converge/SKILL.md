...
- **First spawn:** `Agent(subagent_type: "clean-coder", name:
  "prc-fix-<PR#>", model: sonnet)` — worker-model routing per the
  orchestrator skill; resolver-supplied sonnet-equivalent on third-party
  hosts. The `name` makes the agent a persistent teammate
