...
| `post_audit_thread.py` | Posts an audit review (APPROVE / REQUEST_CHANGES, or COMMENT when the author reviews their own PR with no alternate reviewer account) to a draft PR via the GitHub reviews API; reads the body skeleton from `audit-reply-template.md` at runtime |
