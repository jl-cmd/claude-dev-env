...
## 11.6 STYLE/COMMUNICATION SAVE-PATH AUDIT (2026-09, decision: keep)

A 2026-09 audit asked whether the Style and Communication checks should move off the edit lane (save path) onto the full gate (commit path), the way the six cross-file checks already did. Read this section before repeating the audit.

**Re-derived roster.** Reading every `code_rules_*.py` module and classifying each check by what a violation risks — wrong runtime behavior or a weakened test (Correctness), a broken structural obligation such as a type or docstring-signature agreement (Contract), a naming/formatting convention with no functional risk (Style), or how clearly a docstring communicates (Communication) — gives Correctness 21, Contract 30, Style 27, Communication 2 (80 total). Scope re-derivation confirms exactly six wider (cross-file) checks and 74 single-file checks, matching `ALL_FULL_GATE_ONLY_CHECK_NAMES`; a repository-wide search for sibling-file reads (`glob`, `os.walk`, `iterdir`, `rglob`, `listdir`) across every `code_rules_*.py` module turns up only the four modules that already carry a full-gate-only check.

**Coverage is not the blocker.** `git-hooks/pre_commit.py` invokes `_shared/pr-loop/scripts/code_rules_gate.py --immediate`, which calls `load_validate_content_for_full_gate()` — a strict superset of the edit lane (all 74 edit-lane checks plus the six full-gate-only checks). Every Style and Communication check already runs again at commit time. Moving them off the save path costs zero coverage — unlike the `hooks/validators/` family, where a repository search finds no commit-path script that imports it at all.

**Latency does not justify the move.** `hooks/validators/hook_timing_harness.py` cannot measure this roster's real cost: it always builds a `Write` tool payload against a target file that already has on-disk content, and `code_rules_enforcer.py`'s own Write branch returns `None` (skips validation entirely) whenever `old_content` is non-empty (`_contents_for_validation`). Timing a real `Edit` payload against a real 733-line production file (`_shared/pr-loop/scripts/terminology_sweep.py`) instead: the enforcer subprocess alone costs roughly 760-805ms per save (5 runs); timing each check function directly attributes only 13-16% of that (roughly 104-111ms) to the 27 Style checks. One Contract check, `check_same_file_inline_duplicate_body`, alone costs roughly 500ms — about 62% of the total — and its cost grows with file size. That check, not the Style/Communication split, is the real latency lever, and it is outside this audit's scope.

**Denial frequency is not usable evidence.** `~/.claude/logs/hook-blocks.log` names the failing check in its `reason` text and held 531 `code_rules_enforcer.py` entries at audit time, but every one of the roughly 406 distinct denial events traces to a fixture inside `test_code_rules_enforcer_*.py` or to the timing harness's own default target, not to an organic edit. Style messages made up about 86% of that synthetic corpus; that share describes the test suite's fixture mix, not real editing friction, and does not decide this audit.

**Decision: keep Style and Communication checks on the edit lane.** No coverage gain, a real but secondary latency cost (13-16% of the enforcer's own time, itself a small share of total save time), and no usable denial-frequency evidence justify carving out roughly 29 more checks the way the six cross-file checks were carved out. Revisit only if `check_same_file_inline_duplicate_body`'s cost is fixed first and the remaining edit-lane cost still warrants a split, or if a real per-check denial log accumulates outside test and benchmark runs.
