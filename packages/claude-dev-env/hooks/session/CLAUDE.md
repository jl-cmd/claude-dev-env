...
| `issue_tracker_session_starter.py` | SessionStart | Injects a directive telling Claude to start the issue-tracker skill or agent, gated on an inject-eligible source, the `CLAUDE_ISSUE_TRACKER` toggle, a GitHub origin remote in the session repository, and the tracker skill and agent files existing under `~/.claude`. Emits the nested SessionStart payload; stays silent on any failed gate. |
...
| `_path_setup.py` | ΓÇö | Inserts the hooks directory on `sys.path` so the session entry-point hooks import `hooks_constants` with imports kept at module top. |
| `test_gh_pr_author_session_cleanup.py` | ΓÇö | Tests for `gh_pr_author_session_cleanup.py` |
| `test_session_edit_tracker_cleanup.py` | ΓÇö | Tests for `session_edit_tracker_cleanup.py` |
| `test_session_env_cleanup.py` | ΓÇö | Tests for `session_env_cleanup.py` |
| `test_untracked_repo_detector.py` | ΓÇö | Tests for `untracked_repo_detector.py` |
| `test_task_list_loop_starter.py` | ΓÇö | Tests for `task_list_loop_starter.py` |
| `test_issue_tracker_session_starter.py` | ΓÇö | Tests for `issue_tracker_session_starter.py` |
| `test_orchestrator_auto_starter.py` | ΓÇö | Tests for `orchestrator_auto_starter.py` |
| `test__path_setup.py` | ΓÇö | Tests for `_path_setup.py` |
