...
| `session_edit_stage_gate_constants.py` | Tracker filename prefix/suffix, JSON payload key, edit tool name set, session-id sanitize pattern, git diff command, commit flag escapes, and deny-message template shared by the session edit stage gate trio |
