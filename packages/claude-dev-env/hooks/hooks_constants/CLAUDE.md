...
| `ruff_integration_constants.py` | Environment variable names (`NO_COLOR`, `FORCE_COLOR`) and the enabled value that force plain, non-ANSI ruff diagnostics so the gate parses `path:line:col:` prefixes |
