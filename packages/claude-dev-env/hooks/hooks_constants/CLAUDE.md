...
| `issue_tracker_commit_reminder_constants.py` | Git commit/push token match tokens, shell command-separator tokens, path separators, tool-input payload keys, PreToolUse allow-payload keys and values, and the reminder additionalContext text for the issue-tracker commit/push reminder hook |
