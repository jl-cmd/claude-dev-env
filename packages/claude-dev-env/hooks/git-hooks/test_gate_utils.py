...
def test_resolve_gate_script_path_defaults_to_claude_home_when_env_var_set(
    ...
    expected_path = (
        tmp_path / "skills" / "_shared" / "pr-loop" / "scripts" / "code_rules_gate.py"
    )
...
def test_is_safe_regular_file_rejects_gate_outside_home_dot_claude_tree(
    ...
    gate_under_attacker_home = (
        attacker_home / "skills" / "_shared" / "pr-loop" / "scripts" / "code_rules_gate.py"
    )
...
def test_is_safe_regular_file_accepts_gate_inside_home_dot_claude(
    ...
    gate_path = (
        home_dir / ".claude" / "skills" / "_shared" / "pr-loop" / "scripts" / "code_rules_gate.py"
    )
...
def test_is_safe_regular_file_rejects_nonexistent_path_under_trusted_prefix(
    ...
    missing_gate_path = (
        home_dir / ".claude" / "skills" / "_shared" / "pr-loop" / "scripts" / "missing_gate.py"
    )
    monkeypatch.setattr(Path, "home", staticmethod(lambda: home_dir))
...
def test_is_safe_regular_file_uses_claude_home_env_as_trust_root(
    ...
    gate_path = (
        custom_claude_home / "skills" / "_shared" / "pr-loop" / "scripts" / "code_rules_gate.py"
    )
    gate_path.parent.mkdir(parents=True)
    gate_path.write_text("", encoding="utf-8")
    monkeypatch.delenv("CODE_RULES_GATE_PATH", raising=False)
