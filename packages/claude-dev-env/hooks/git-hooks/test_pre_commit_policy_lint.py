"""Isolated checks for the local staged policy-lint owner."""
...
def _lint_report(repository_root: Path) -> tuple[int, dict[str, object]]:
    completed = subprocess.run(
        [sys.executable, str(pre_commit.resolve_policy_lint_script_path()),
         "--staged", "--format", "json"],
        cwd=repository_root,
        check=False,
        capture_output=True,
...
def test_real_staged_linter_catches_each_replaced_rule(
    repository_root: Path, rule_id: str, relative_path: str, content: str,
) -> None:
    ...
    assert rule_id in {each["rule_id"] for each in report["diagnostics"]}, report
...
def test_missing_linter_does_not_allow_commit(
    ...
    monkeypatch.setattr(pre_commit, "resolve_policy_lint_script_path",
...
def test_linter_failure_reaches_git_before_legacy_gate(
    ...
    monkeypatch.setattr(pre_commit, "resolve_gate_script_path",
