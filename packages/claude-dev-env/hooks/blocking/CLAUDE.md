...
| `plain_language_blocker.py` | PreToolUse (Write/Edit/AskUserQuestion) | Heavy or jargon words in user-facing prose; an `AskUserQuestion` block whose question text or choice description carries chat detail — a line-opening fenced code block, heading, table row, or bullet or numbered list marker, a second paragraph, or prose past the sentence and word caps, with line endings folded to one spelling first and an inline code span weighing one word against the length caps |
