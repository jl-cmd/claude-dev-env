...
| `code_verifier_spawn_preflight_gate.py` | PreToolUse (Agent\|Task) | Spawning the `code-verifier` subagent when the branch has a merge conflict vs its base, a CODE_RULES violation on a working-tree-added line, or the CODE_RULES engine fails to load |
