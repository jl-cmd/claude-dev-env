...
def test_multi_edit_denies_when_the_first_edit_writes_the_stale_comment(
    tmp_path: Path,
) -> None:
    """The comment the second edit orphans is one the first edit just wrote.

    A non-cumulative check reads the ORIGINAL "# Old note" comment and finds
    nothing to orphan; only applying the first edit before the second reveals
    "# Mock asyncio" above the removed line.
    """
    written_module_path = _write_module(
        tmp_path,
        "def launch_export() -> None:\n    # Old note\n" + PATCHED_SLEEP_LINE,
...
def test_multi_edit_allows_two_unrelated_edits(tmp_path: Path) -> None:
    written_module_path = _write_module(
        tmp_path,
        "def launch_export() -> None:\n" + KEPT_COMMENT_LINE + PATCHED_SLEEP_LINE,
    )
    all_edits = [
        {
            "old_string": "def launch_export() -> None:",
            "new_string": "def launch_export_now() -> None:",
        },
        {
            "old_string": PATCHED_SLEEP_LINE,
