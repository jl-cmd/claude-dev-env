...
DRAFT_PR_INSTRUCTION = (
    " Feature branch required: (1) create a feature branch with `git checkout -b <descriptive-branch-name>`, "
    "(2) commit your changes there, "
    "(3) push with `git push -u origin <branch-name>`, "
    "(4) create a draft PR with `gh pr create --draft`. "
    "A main-branch commit requires explicit user approval."
)
...
def build_denial_response(branch_name: str, repo_dir: str | None) -> dict:
    ...
    denial_reason = (
        f"BLOCKED: Feature branch required for '{branch_name}'{location}."
        + DRAFT_PR_INSTRUCTION
    )
