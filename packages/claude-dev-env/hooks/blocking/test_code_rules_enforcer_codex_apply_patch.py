...
def test_codex_patch_operation_targets_names_every_section_without_reading_content(
    ...
    all_target_names = {
        (each_operation, Path(each_path).name) for each_operation, each_path in all_targets
    }
    assert all_target_names == {
        ("update", "updated.py"),
        (codex_apply_patch.CODEX_ADD_OPERATION, "added.py"),
        ("delete", "deleted.py"),
    }
...
def test_codex_patch_operation_targets_rejects_path_traversal(tmp_path: Path) -> None:
    ...
    with pytest.raises(codex_apply_patch.CodexPatchError):
        codex_apply_patch.codex_patch_operation_targets(escaping_patch, str(tmp_path))
