...
- A workflow gate-outcome flag is documented as a bypass while the code blocks on it: a schema property `description`, an architecture `detail` string, or a companion reference `.md` bullet says the outcome "skips without blocking" / "the gate is bypassed" / "move to the convergence check", while the branch handling that outcome sets a blocker and `break`s to hold the PR in draft.
...
| O8 | Companion-doc ordering/content vs producer | When a PR changes a producer function's ordering or union, read that skill's companion `SKILL.md` and sibling `.md` docs for any sentence naming the same produced artifact (a file path, a JSON key, a named list). A doc sentence that claims the artifact is `sorted` / `alphabetical` / `in sorted order`, or holds `just the at-risk names` / `only the current set`, while the producer merges stored names with new names and appends — preserving file order, not re-sorting the union — is an O8 finding on both counts (wrong order claim, hidden merged-in entries). The finding stands even when the PR diff never touched the `.md` file, because the behavior change orphaned the doc claim. A producer docstring asserting that no consumer reads its output yet (`producer-only artifact`, `no submission-run consumer reads it yet`) is the deterministic slice of this companion-doc producer/consumer drift (`check_docstring_no_consumer_claim`). When a PR changes a gate outcome's control flow — routing an outcome to a blocker `break` that holds the PR in draft — read every reference `.md` that documents that gate for a bullet describing the same outcome as a bypass (`set codexDown, no stamp, move to the convergence check`); a bullet that still routes the blocked outcome down the bypass path, especially when a sibling reference file already describes it as a blocker, is an O8 finding even when the PR diff never touched that `.md`. |
