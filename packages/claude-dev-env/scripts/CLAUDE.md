...
| `shared_tree_paths.py` | Resolves a directory inside the installed `_shared` tree for a caller module: `resolve_shared_process_tree_scripts_directory` tries the un-resolved parent of the caller first, then the `CLAUDE_CONFIG_DIR` root, then the resolved parent, and returns the first one holding `process_tree_kill.py`. The un-resolved parent comes first so an install whose `scripts` directory is a junction finds `_shared` beside the managed root rather than beside the link target |
