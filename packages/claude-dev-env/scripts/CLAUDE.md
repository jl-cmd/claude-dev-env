...
| `grok_batch_scaffold.py` | Writes the deterministic worker part files (the shared report contract, a per-worker brief, and a per-worker task-body placeholder) and a wired batch-spec skeleton under a run directory, so the caller only fills task scope before launching `spawn_grok_batch.py` |
...
| `grok_worker_preflight.py` | Soft gate for the headless grok tier: binary on PATH, `grok models` auth, and the `claude-dev-env` install manifest, plus an opt-in cached live ping; a non-zero exit signals fallthrough to the next tier |
