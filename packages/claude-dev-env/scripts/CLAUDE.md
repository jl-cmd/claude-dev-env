...
| `invoke_code_review.py` | Host-aware `/code-review` invoker with effort token and optional stamp mint: in-session on Claude+opus, headless via `claude_chain_runner` otherwise; `--session-has-usage-left false` forces chain when the primary session is drained |
