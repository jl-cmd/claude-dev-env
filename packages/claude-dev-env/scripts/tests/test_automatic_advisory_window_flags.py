...
from automatic_advisory import checkout, cli, execution, git
...
def test_flags_are_zero_off_windows(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(subprocess_window.sys, "platform", "linux")
    assert subprocess_window.hidden_window_creation_flags() == 0
    assert subprocess_window.detached_hidden_window_creation_flags() == 0


@WINDOWS_ONLY
def test_hidden_window_creation_flags_is_create_no_window() -> None:
    assert subprocess_window.hidden_window_creation_flags() == subprocess.CREATE_NO_WINDOW


@WINDOWS_ONLY
def test_poller_flags_keep_a_hidden_console_instead_of_none() -> None:
    poller_flags = subprocess_window.detached_hidden_window_creation_flags()
...
def test_poller_grandchild_gets_no_visible_console(tmp_path: Path) -> None:
    ...
    subprocess.Popen(
        ...
        creationflags=subprocess_window.detached_hidden_window_creation_flags(),
    )
...
def test_git_fetch_hides_its_window(
    ...
    assert (
        captured_options["creationflags"] == subprocess_window.hidden_window_creation_flags()
    )
...
def test_checkout_inspection_hides_its_window(
    ...
    assert all_captured_flags == [subprocess_window.hidden_window_creation_flags()] * 2
...
def test_windows_process_host_spawn_hides_its_window(
    ...
    expected_flags = subprocess_window.hidden_window_creation_flags() | getattr(
...
def test_start_polling_launches_with_the_poller_flags(
    ...
    assert (
        ...
        == subprocess_window.detached_hidden_window_creation_flags()
    )
