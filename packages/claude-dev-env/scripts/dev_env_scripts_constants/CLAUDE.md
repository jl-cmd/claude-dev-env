...
| `post_claude_review_clean_comment_constants.py` | `post_claude_review_clean_comment.py` - clean marker title, body line templates, gh argv tokens, issue-comments API path, CLI flags, soft-fail result keys, and outcome messages |
| `claude_usage_probe_constants.py` | `claude_usage_probe.py` - session no-usage threshold, subprocess timeout, usage-pause script path parts, probe report JSON keys, unavailable source label, and probe CLI exit code |
