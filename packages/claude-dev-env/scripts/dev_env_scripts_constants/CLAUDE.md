...
| `code_review_constants.py` | `invoke_code_review.py` - the `/code-review ultra --fix` prompt, opus model alias, permission-mode flag and value, the output signatures that mark a host as refusing that permission mode, result mode and JSON keys, session-model CLI flag, git dirty-check tokens, and in-session return markers |
