...
| `code_review_constants.py` | `invoke_code_review.py` - effort tokens and defaults, `/code-review` slash and fix flags, opus model alias, permission-mode flag and value, result mode and JSON keys, session-model CLI flag, session-has-usage-left CLI tokens, stamp-mint keys and return codes, git dirty-check tokens, and in-session return markers |
