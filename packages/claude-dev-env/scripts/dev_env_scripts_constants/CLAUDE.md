...
| `code_review_constants.py` | `invoke_code_review.py` - the `/code-review xhigh --fix` prompt, opus model alias, permission-mode flag and value, result mode and JSON keys, session-model CLI flag, session-has-usage-left CLI tokens, git dirty-check tokens, and in-session return markers |
| `post_claude_review_clean_comment_constants.py` | `post_claude_review_clean_comment.py` - clean marker title, body line templates, gh argv tokens, issue-comments API path and page size, CLI flags, soft-fail result keys, outcome messages, and the subprocess timeout, launch-failure return code, and decode-error policy that keep a gh flake on the soft-fail path |
| `claude_usage_probe_constants.py` | `claude_usage_probe.py` - session no-usage threshold, subprocess timeout and decode-error policy, usage-pause script path parts, probe report JSON keys, unavailable source label, and probe CLI exit code |
