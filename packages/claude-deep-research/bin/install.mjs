...
const MANIFEST_FILE = join(CLAUDE_HOME, '.claude-deep-research-manifest.json');
const PACKAGE_NAME = 'claude-deep-research';
...
function install() {
    ...
    const agentsSource = join(PACKAGE_ROOT, 'agents');
    ...
    console.log(`\nInstalled ${PACKAGE_NAME}: ${allInstalledFiles.length} files\n`);
}
...
function printHelp() {
    console.log(`
${PACKAGE_NAME} - Claude Code deep research installer

Usage:
  npx ${PACKAGE_NAME}              Install research skills and agent
  npx ${PACKAGE_NAME} --uninstall  Remove installed files
  npx ${PACKAGE_NAME} --help       Show this help

Install location: ~/.claude/skills/ and ~/.claude/agents/
`);
}
