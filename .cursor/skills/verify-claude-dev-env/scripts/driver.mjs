 ...
 */
export function resolveDriverPaths(driverDirectory) {
    const repositoryRoot = resolve(driverDirectory, '..', '..', '..', '..');
    const packageRoot = join(repositoryRoot, 'packages', 'claude-dev-env');
    return {
        repositoryRoot,
        packageRoot,
        installEntry: join(packageRoot, 'bin', 'install.mjs'),
    };
}

const DRIVER_DIRECTORY = dirname(fileURLToPath(import.meta.url));
const { packageRoot: PACKAGE_ROOT, installEntry: INSTALL_ENTRY } = resolveDriverPaths(DRIVER_DIRECTORY);
...
function runInstaller(sandboxHome, installerArguments) {
    ...
    const installerRun = spawnSync(process.execPath, [INSTALL_ENTRY, ...installerArguments], {
        cwd: PACKAGE_ROOT,
        encoding: 'utf8',
        env: {
            ...process.env,
