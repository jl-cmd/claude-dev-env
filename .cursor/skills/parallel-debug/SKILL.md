...
Converge every open PR in `jl-cmd/claude-dev-env` to back-to-back clean: bugbot CLEAN + bugteam CLEAN at the same HEAD. Mark each converged PR ready for review with `gh pr ready`. Stop when all open PRs have converged or hit a hard blocker.
...
**Open PRs:**
```bash
gh api 'repos/jl-cmd/claude-dev-env/pulls?state=open&per_page=100' --paginate --slurp \
...
python "$HOME\.claude\skills\pr-converge\scripts\fetch_bugbot_reviews.py" \
  --owner jl-cmd --repo claude-dev-env --number <N>
```
...
python "$HOME\.claude\skills\pr-converge\scripts\fetch_bugbot_inline_comments.py" \
  --owner jl-cmd --repo claude-dev-env --number <N> --commit <HEAD_SHA>
```

**Prior bugteam review per PR** (run for each `<N>`):
```bash
gh api 'repos/jl-cmd/claude-dev-env/pulls/<N>/reviews?per_page=100' --paginate --slurp \
...
1. List open PR numbers, for example:
   ```bash
   gh pr list -R jl-cmd/claude-dev-env --state open --json number -q ".[].number"
   ```
...
```bash
gh pr view <N> -R jl-cmd/claude-dev-env --json "number,url,headRefOid,baseRefName,headRefName,isDraft"
```
...
- Fetch inline findings for current HEAD:
  ```bash
  python "$HOME\.claude\skills\pr-converge\scripts\fetch_bugbot_inline_comments.py" \
    --owner jl-cmd --repo claude-dev-env --number <N> --commit <current_head>
  ```
- Decide:
...
gh pr comment <N> --repo jl-cmd/claude-dev-env --body-file $bodyPath
...
7. Reply to each addressed inline comment thread:
   ```bash
   gh api -X POST repos/jl-cmd/claude-dev-env/pulls/<N>/comments/<comment_id>/replies \
...
```bash
gh pr ready <N> --repo jl-cmd/claude-dev-env
```
...
- All open PRs in `jl-cmd/claude-dev-env` have been marked ready for review via `gh pr ready`
