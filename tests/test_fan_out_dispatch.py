...
PLACEHOLDER_OWNER_SCOPES = "example-owner,example-org"


@pytest.fixture(autouse=True)
def _set_fanout_owner_scopes_env() -> Iterator[None]:
    with patch.dict(
        os.environ, {FANOUT_OWNER_SCOPES_ENV_VAR: PLACEHOLDER_OWNER_SCOPES}, clear=False
    ):
        yield


def make_repo_fixture(
    full_name: str = "example-owner/some-repo",
    archived: bool = False,
    has_push: bool = True,
    is_fork: bool = False,
) -> dict:
...
class TestIsTargetRepo:
    def should_include_jonecho_repos_with_push(self) -> None:
        repo = make_repo_fixture("example-owner/my-project")

        assert fan_out_dispatch.is_target_repo(repo) is True

    def should_include_jlcmd_repos_with_push(self) -> None:
        repo = make_repo_fixture("example-org/some-tool")

        assert fan_out_dispatch.is_target_repo(repo) is True

    def should_exclude_archived_repos(self) -> None:
        repo = make_repo_fixture("example-owner/old-project", archived=True)
    ...
    def should_include_repos_even_when_permissions_push_is_absent(self) -> None:
        repo = make_repo_fixture("example-owner/read-only", has_push=False)

        assert fan_out_dispatch.is_target_repo(repo) is True

    def should_include_the_source_repo_for_bugbot_sync(self) -> None:
        repo = make_repo_fixture("example-org/claude-dev-env")
    ...
    def should_include_forks_when_owner_is_jonecho(self) -> None:
        repo = make_repo_fixture("example-owner/forked-repo", is_fork=True)

        assert fan_out_dispatch.is_target_repo(repo) is True

    def should_include_forks_when_owner_is_jlcmd(self) -> None:
        repo = make_repo_fixture("example-org/forked-repo", is_fork=True)
...
class TestBuildSummaryTable:
    ...
    def should_report_counts_without_repo_names(self) -> None:
        dispatch_status_by_repo = {
            "example-owner/alpha": fan_out_dispatch.DISPATCH_STATUS_SUCCEEDED,
            "example-owner/beta": fan_out_dispatch.DISPATCH_STATUS_OPTED_OUT,
        }

        table = fan_out_dispatch.build_summary_table(dispatch_status_by_repo, {})

        assert "example-owner/alpha" not in table
        assert "example-owner/beta" not in table
    ...
    def should_count_listener_conclusions_by_status(self) -> None:
        dispatch_status_by_repo = {
            "example-owner/alpha": fan_out_dispatch.DISPATCH_STATUS_SUCCEEDED,
            "example-owner/beta": fan_out_dispatch.DISPATCH_STATUS_SUCCEEDED,
        }
        conclusion_by_repo = {
            "example-owner/alpha": fan_out_dispatch.LISTENER_CONCLUSION_SUCCESS,
            "example-owner/beta": fan_out_dispatch.LISTENER_CONCLUSION_FAILURE,
        }
    ...
    def should_count_dispatch_failures(self) -> None:
        dispatch_status_by_repo = {
            "example-owner/alpha": fan_out_dispatch.DISPATCH_STATUS_FAILED,
        }
...
class TestBuildStaleSection:
    ...
    def should_report_stale_count_without_repo_names(self) -> None:
        stale_repos = ["example-owner/old-project", "example-org/abandoned"]

        section = fan_out_dispatch.build_stale_section(stale_repos)

        assert "2 target repo(s)" in section
        assert "example-owner/old-project" not in section
        assert "example-org/abandoned" not in section

    def should_include_stale_section_heading(self) -> None:
        section = fan_out_dispatch.build_stale_section(["example-owner/some-repo"])

        assert "Stale listeners" in section


FAKE_TOKEN = "ghs_fake_installation_token"
FAKE_OWNER = "example-owner"
FAKE_REPO = "some-repo"
...
class TestExitCodeFailsOnListenerProblems:
    ...
    def should_exit_zero_when_every_dispatch_succeeded_and_every_conclusion_succeeded(
        ...
        exit_code = fan_out_dispatch.compute_exit_code(
            dispatch_status_by_repo={
                "example-org/one": fan_out_dispatch.DISPATCH_STATUS_SUCCEEDED,
                "example-org/two": fan_out_dispatch.DISPATCH_STATUS_SUCCEEDED,
            },
            conclusion_by_repo={
                "example-org/one": fan_out_dispatch.LISTENER_CONCLUSION_SUCCESS,
                "example-org/two": fan_out_dispatch.LISTENER_CONCLUSION_SUCCESS,
            },
        )
    ...
    def should_exit_nonzero_when_any_dispatch_failed(self) -> None:
        exit_code = fan_out_dispatch.compute_exit_code(
            dispatch_status_by_repo={
                "example-org/one": fan_out_dispatch.DISPATCH_STATUS_SUCCEEDED,
                "example-org/two": fan_out_dispatch.DISPATCH_STATUS_FAILED,
            },
            conclusion_by_repo={
                "example-org/one": fan_out_dispatch.LISTENER_CONCLUSION_SUCCESS,
            },
    ...
    def should_exit_nonzero_when_any_listener_conclusion_failed(self) -> None:
        exit_code = fan_out_dispatch.compute_exit_code(
            ...
            conclusion_by_repo={
                "example-org/one": fan_out_dispatch.LISTENER_CONCLUSION_FAILURE,
            },
        )
    ...
    def should_exit_nonzero_when_any_listener_conclusion_is_pending(self) -> None:
        exit_code = fan_out_dispatch.compute_exit_code(
            ...
            conclusion_by_repo={
                "example-org/one": fan_out_dispatch.LISTENER_STATUS_PENDING,
            },
        )
    ...
    def should_exit_nonzero_when_any_listener_conclusion_is_poll_error(self) -> None:
        exit_code = fan_out_dispatch.compute_exit_code(
            ...
            conclusion_by_repo={
                "example-org/one": fan_out_dispatch.LISTENER_STATUS_POLL_ERROR,
            },
        )
...
class TestScriptEntrypointImportPath:
    def should_load_config_when_run_as_python_script_with_empty_pythonpath(
        ...
        env = os.environ.copy()
        env["PYTHONPATH"] = ""
        env["EXAMPLEOWNER_TOKEN"] = ""
        env["EXAMPLEORG_TOKEN"] = ""
...
class TestMalformedRepoEntriesAreReported:
    def should_warn_with_a_count_when_a_target_entry_is_malformed(self) -> None:
        ...
        token_by_owner = {"example-owner": FAKE_TOKEN}
...
class TestExcludedRepoCountNoticeIsGuarded:
    def should_not_emit_the_notice_when_no_repo_is_excluded(self) -> None:
        all_target_repos = [
            make_repo_fixture("example-owner/first"),
            make_repo_fixture("example-org/second"),
        ]
        with patch.object(
            fan_out_dispatch,
    ...
    def should_emit_the_notice_with_a_count_when_a_repo_is_excluded(self) -> None:
        all_candidate_repos = [
            make_repo_fixture("example-owner/kept"),
            make_repo_fixture("example-owner/dropped", archived=True),
        ]
        with patch.object(
            fan_out_dispatch,
            "_collect_candidate_repos",
...
class TestEnumerationLoggingRoutesThroughDispatchLogger:
    ...
    def should_log_repository_count_through_the_dispatch_logger(self) -> None:
        one_repo_page = {"repositories": [make_repo_fixture("example-owner/one")]}
        with patch.object(
            fan_out_dispatch,
            "make_github_api_request",
