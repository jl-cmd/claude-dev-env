...
| `test_session_start_refresh.py` | Runs the `.claude/hooks/session_start_refresh.py` SessionStart hook as a subprocess with fake `npm`/`npx` shims on `PATH`: the remote-session guard, the version-difference reinstall, the home-directory working directory, and the fail-open registry probe. |
