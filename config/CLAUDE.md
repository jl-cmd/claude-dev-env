...
| `local-identity.example.json` | Committed template for the git-ignored `config/local-identity.json`; carries placeholder owner scopes, NAS host, user, and port, and a per-repository PII allowlist mapping (`pii_allowlisted_values`). |
