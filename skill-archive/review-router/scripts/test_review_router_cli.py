...
from review_router_constants.config.constants import UNSUPPORTED_ROUTE
...
def test_resolve_returns_unsupported_for_unknown_route(tmp_path: Path) -> None:
    resolve_response = resolve(str(tmp_path), "unknown-route", "", None)
    assert resolve_response["status"] == UNSUPPORTED_ROUTE
