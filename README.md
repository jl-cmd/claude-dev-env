...
| `advisor` | Turns the session into the advisor-orchestrator (the user's sole interface): it spawns executor subagents to do all code edits and test runs, and answers a blocked executor with a plan, correction, or stop; caps consultations and reuses warm agents before spawning new ones |
| `advisor-refresh` | Sub-skill fired by the `/advisor` loop about every 20 minutes to re-assert the executor-advisor discipline mid-run |
