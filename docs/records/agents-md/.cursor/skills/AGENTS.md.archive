...
- Skills here are Cursor-session versions of the Claude Code skills in `packages/claude-dev-env/.agents/skills/`. They share the same convergence goal but adapt to Cursor's session model (AHK pacing; no `ScheduleWakeup`).
