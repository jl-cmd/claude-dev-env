...
`jl-cmd/claude-dev-env` is the single source of truth for AI code-review rules.
...
In **`jl-cmd/claude-dev-env` only**, the canonical AGENTS file is edited on `main` directly;
...
jl-cmd/claude-dev-env
  ...
  .github/workflows/fan-out-ai-rules.yml  (dispatcher)
           ...
           ▼
  Each target repo (including jl-cmd/claude-dev-env):
...
**Always edit `AGENTS.md` in `jl-cmd/claude-dev-env`.**
...
Revert the source file in `jl-cmd/claude-dev-env` and push to `main`.
...
```bash
gh workflow run fan-out-ai-rules.yml -R jl-cmd/claude-dev-env
```
...
5. In `jl-cmd/claude-dev-env` repository secrets, set:
