...
| `pwsh` | Absent. |
| GitHub MCP server | Available. The MCP login follows the account connection: the account owner runs cloud sessions as `example-user` or `example-org-2`. Read the live login with `mcp__github__get_me` at run start (a probe against this doc's session read `example-org-2`, id 100000001). Section 7 stamps each probe with the login it ran under. Reads work on `example-org-2/claude-dev-env` and `example-org/example-repo`. |
...
| `GH_TOKEN` / `GITHUB_TOKEN` | Both set, holding one value that starts with `prox` — a proxy placeholder. The agent proxy swaps in real credentials on outbound HTTPS. Two identities run side by side: MCP tools act as the user the MCP login names (`example-org-2` in this doc's session), while raw REST carrying `GH_TOKEN` posts as the `claude[bot]` GitHub App (app slug `claude`, app id 100001, `author_association` NONE). `GET /user` answers with the MCP user login. A body posted through REST carries a server-appended `Generated by Claude Code` footer. |
...
- **Cause:** The cloud session posts reviews under one MCP identity — the MCP login (`example-org-2` in this doc's session). `BUGTEAM_REVIEWER_ACCOUNT`, `gh auth switch`, and the account-swap helpers have no cloud equivalent, so a run cannot swap to a second reviewer account. GitHub blocks `APPROVE`/`REQUEST_CHANGES` on one's own PR; `COMMENT` reviews stay allowed.
- **Evidence:** `post_audit_thread.py` reads `BUGTEAM_REVIEWER_ACCOUNT` (unset in cloud) and calls `gh auth status` / `gh auth token --user` (RC1). PRs a cloud session opens are authored by `example-org-2`. A PR authored locally by `example-org` can carry a `example-org-2` review normally.
...
- **Symptom:** Raw REST works for `example-org/*` and returns 403 for `example-org-2/*`.
- **Cause:** Proxy REST is served only for owners the Claude GitHub App connection covers.
- **Evidence:** `GET /repos/example-org/example-repo` returns 200; `GET /repos/example-org-2/claude-dev-env` returns 403 with the app-not-connected body (Section 2).
- **Fix summary:** U1 is the account-owner action that connects the app for `example-org-2`. Until then, `example-org-2/*` GitHub work runs through MCP, which reads both repos.
...
- **Fix summary:** Resolve the available runtime at execution time and keep cloud review output in the retired `reviews` skill guide format.
...
| Second reviewer identity | `gh auth switch`, `BUGTEAM_REVIEWER_ACCOUNT` | None. One `example-org-2` identity. `COMMENT` reviews on own PRs allowed; `APPROVE`/`REQUEST_CHANGES` on own PRs blocked. |
| REST fallback | `gh api repos/...` | `curl -H "Authorization: Bearer $GH_TOKEN" https://api.github.com/repos/...` — only for owners with the app connected (`example-org` today). GraphQL is pinned. |
...
- Use raw REST only for owners the Claude GitHub App connection covers
  (`example-org` today). A `example-org-2/*` REST call returns 403 until U1 lands.
  REST client:
...
- The session holds one MCP identity. Read it with `mcp__github__get_me` at
  run start; the account owner runs cloud sessions as `example-user` or `example-org-2`.
...
- On a PR another account authored (for example a `example-org` PR when the MCP
  login is `example-org-2`), a review of any type from the MCP login is allowed.
...
- Review comments follow the retired `reviews` skill guide.
...
probes is `example-org-2`; raw REST posts as `claude[bot]`.
...
| `mcp__github__get_me` identity | example-org-2 (MCP) | `example-org-2` (id 100000001) | verified |
| MCP read on `example-org-2/claude-dev-env` | example-org-2 (MCP) | Works | verified |
| MCP read on `example-org/example-repo` | example-org-2 (MCP) | Works | verified |
| `mcp__github__*` call before `ToolSearch` load | example-org-2 (MCP) | `InputValidationError` | verified |
| `GET https://api.github.com/user` | claude[bot] (REST) | 200, login `example-org-2` | verified |
| `GET /repos/example-org/example-repo` | claude[bot] (REST) | 200 | verified |
| `GET /repos/example-org/example-repo` viewer `permissions` object | claude[bot] (REST) | Present, with a `push` boolean (`false` for this token) | verified |
| `GET /repos/example-org-2/claude-dev-env` | claude[bot] (REST) | 403, app-not-connected body | verified |
...
| MCP `mcp__github__issue_write(method="create")` on `example-org-2/claude-dev-env` | example-org-2 (MCP) | Issue #102 (id 400000002) created | verified |
| MCP `mcp__github__add_issue_comment` on issue #102 | example-org-2 (MCP) | Comment id 490000002 | verified |
| MCP `mcp__github__issue_write(method="update", state="closed")` on #102 | example-org-2 (MCP) | Read-back state closed, `state_reason` not_planned, `closed_by` example-org-2 | verified |
| REST issue create on `example-org/example-repo` | claude[bot] (REST) | HTTP 201, issue #101 (id 400000001), author `claude[bot]` | verified |
| REST issue comment on #101 | claude[bot] (REST) | HTTP 201, comment id 490000001 | verified |
| REST issue close (PATCH state=closed) on #101 | claude[bot] (REST) | HTTP 200, read-back state closed, `state_reason` completed, `closed_by` claude[bot] | verified |
| REST read or write on `example-org-2/claude-dev-env` | claude[bot] (REST) | 403 app-not-connected body; repo-scoped, so reads and writes alike refuse | verified |
...
| Owner-wide `mcp__github__search_pull_requests` | example-org-2 (MCP) | `user:example-org is:open` 19; `user:example-org-2 is:open` 15. Always pass `perPage`: an unpaginated owner-wide search overflows the tool-result limit. | verified |
| PR comment via `mcp__github__add_issue_comment` on PR #103 | example-org-2 (MCP) | Comment id 490000003 | verified |
| Pending-review flow (create pending, add inline comment, send the `COMMENT` review) on own PR #103 | example-org-2 (MCP) | All three steps land; review id 460000001; a `COMMENT` review lands on the author's own PR | verified |
| Reply to an inline comment on PR #103 | example-org-2 (MCP) | `mcp__github__pull_request_read(method="get_review_comments")` exposes `discussion_r350000001`; `mcp__github__add_reply_to_pull_request_comment(commentId=350000001)` returns reply id 350000002. The numeric comment id surfaces only inside the `html_url` `discussion_r` anchor of `mcp__github__pull_request_read(method="get_review_comments")` output. | verified |
| Thread resolve, unresolve, re-resolve on PR #103 | example-org-2 (MCP) | All three steps land; final read-back `is_resolved=true`, thread `PRRT_kwDOEXAMPLE0001` | verified |
| `mcp__github__request_copilot_review` on PR #103 | example-org-2 (MCP) | Completes with no output in both draft and ready states; no in-band confirmation either way. A landed Copilot review is the only confirming signal. | verified |
| `mcp__github__update_pull_request(draft=false/true)` toggle on PR #103 | example-org-2 (MCP) | To ready: draft `false`, `mergeable_state` clean. Back to draft: draft `true` (read-back). | verified |
| `bugbot run` trigger on a `example-org-2/claude-dev-env` PR | example-org-2 (MCP) posts; cursor[bot] answers | Comment posts (id 490000004). `cursor[bot]` answers with both denials: "Skipping Bugbot: Unable to authenticate your request. Please make sure Bugbot is properly installed and configured for this repository." and "Skipping Bugbot: Bugbot is disabled for this repository. Visit the Bugbot dashboard to update your settings." No bugbot check run appears. | verified |
...
**U1. Connect the Claude GitHub App for the `example-org-2` account or org.**
This makes proxy REST work for `example-org-2/*` (it fixes the blocked half of RC5)
and turns on the script-level REST fallback everywhere.

**U2. Decide the Copilot strategy for cloud.**
Either request Copilot as `example-org-2` (which needs Copilot access on `example-org-2`,
or repo-level Copilot review turned on) or accept `copilot_down` in cloud
runs.

**U3. Optional second-identity strategy.**
Add a fine-grained PAT for `example-org` (or a second `example-org-2` reviewer) as an
...
**U5. Enable Bugbot for `example-org-2/claude-dev-env`.**
