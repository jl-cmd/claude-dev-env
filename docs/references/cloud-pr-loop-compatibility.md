...
| `pwsh` | Absent. |
| GitHub MCP server | Available. Identity from `mcp__github__get_me` is `jl-cmd` (id 149310511). Reads work on `jl-cmd/claude-code-config` and `JonEcho/python-automation`. |
...
| `GH_TOKEN` / `GITHUB_TOKEN` | Both set, holding one value that starts with `prox` — a proxy placeholder. The agent proxy swaps in real credentials on outbound HTTPS. |
...
| Request the Copilot reviewer | `gh api POST pulls/N/requested_reviewers` | `mcp__github__request_copilot_review` |
| Mark ready / send to draft | `gh pr ready`, `gh pr ready --undo` | `update_pull_request(draft=false / draft=true)` |
...
| Cross-repo PR search | `gh search prs --owner X --state open` | `search_pull_requests(query="user:X is:open")` |
...
The `request_copilot_review` and `update_pull_request(draft=...)` rows carry
a `pending` mark in Section 7 until a probe against a scratch PR confirms
them.
...
- The session holds one identity: `jl-cmd`. Confirm it with
  `mcp__github__get_me`.
- On a PR the session authored, post `COMMENT` reviews. GitHub blocks
  `APPROVE` and `REQUEST_CHANGES` on one's own PR.
- On a PR another account authored (for example a `JonEcho` PR), a `jl-cmd`
  review of any type is allowed.
...
Rows marked `verified` rest on a live probe in a cloud session. Rows marked
`pending` need a probe against a scratch PR once this document's draft PR
exists.

| Probe | Result | Status |
|---|---|---|
| `command -v gh` | Fails (absent) | verified |
| `git`, `python3`, `python`, `jq`, `node`, `curl` present | All present | verified |
| `pwsh` present | Absent | verified |
| `mcp__github__get_me` identity | `jl-cmd` (id 149310511) | verified |
| MCP read on `jl-cmd/claude-code-config` | Works | verified |
| MCP read on `JonEcho/python-automation` | Works | verified |
| `mcp__github__*` call before `ToolSearch` load | `InputValidationError` | verified |
| `GET https://api.github.com/user` | 200 as `jl-cmd` | verified |
| `GET /repos/JonEcho/python-automation` | 200 | verified |
| `GET /repos/jl-cmd/claude-code-config` | 403, app-not-connected body | verified |
| `GET /repos/cli/cli/releases/latest` | 403 (out-of-scope repo) | verified |
| Non-pinned GraphQL query | Pinned-set rejection body | verified |
| `git push` dry-run before `git remote set-head origin -a` | Fails: `Not a valid object name origin/HEAD` | verified |
| `git push` dry-run after `git remote set-head origin -a` | Succeeds | verified |
| `ScheduleWakeup` in the main session | Available | verified |
| `EnterWorktree`, `Monitor` available | Available (`Monitor` deferred) | verified |
| `BUGTEAM_REVIEWER_ACCOUNT`, `CLAUDE_REVIEWS_DISABLED` unset | Both unset | verified |
| `mcp__github__request_copilot_review` on a scratch PR | — | pending |
| `mcp__github__update_pull_request(draft=false/true)` toggle | — | pending |
| `COMMENT` review on an own (`jl-cmd`) PR | — | pending |
| `mcp__github__search_pull_requests(query="user:X is:open")` returns open PRs | — | pending |
| Full `/qbug` or `/pr-converge` tick (Phase E) | — | pending |
