...
Each record passes a containment guard first: the path resolves under `~/.claude`, or the purge skips it with a warning and counts it. Skipping keeps one malformed record from stranding the user with a half-removed install — the purge removes every legitimate record, clears the manifest, and reports the skipped count.
