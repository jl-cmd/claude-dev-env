...
- A new test, probe, sweep, mutation check, or measurement script counts as evidence only after that same check ran red on a deliberate named break, with a paired control passing beside it. The shown-red record names three parts: the break applied, the red output it printed, and the control that passed beside it. A green with no shown red is an unmeasured result, not a pass (`falsify-before-green.md`).
