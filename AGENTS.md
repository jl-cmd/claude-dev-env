...
- A per-directory inventory that describes a constants module by listing the constant families it holds — a `CLAUDE.md` `config/` row cell, a `README.md` table cell, or the constants module's own module docstring — names every constant family the module defines. When the module holds a constant family the description leaves out (an `ALL_*` collection, a file-name constant, a path-boundary constant), the description reads as a full map of the module while it covers part of the set, so a reader who trusts the list to name the whole module misses a family. The same edit that adds the constant family extends each description that lists the set — the module docstring and the sibling inventory row move together. Full rule: [`packages/claude-dev-env/rules/package-inventory-stale-entry.md`](packages/claude-dev-env/rules/package-inventory-stale-entry.md).
