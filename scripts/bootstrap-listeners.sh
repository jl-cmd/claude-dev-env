...
SOURCE_REPO="jl-cmd/claude-dev-env"
OPT_OUT_SENTINEL_PATH=".github/sync-ai-rules.optout"

DEFAULT_OWNER_SCOPE="example-owner"
OWNERS=()
DRY_RUN=false
AUTO_MERGE=false
SKIP_REPOS=()
...
Deploys the sync-ai-rules listener to all active repos under the configured owner scopes.

Options:
  --dry-run            Preview changes without creating branches or PRs
  --owner <name>       Limit to a specific owner (repeatable; default: the configured owner scopes)
...
resolve_owner_scopes() {
    if [[ -n "${FANOUT_OWNER_SCOPES:-}" ]]; then
        local -a env_scopes=()
        IFS=',' read -r -a env_scopes <<< "${FANOUT_OWNER_SCOPES}"
        printf '%s\n' "${env_scopes[@]}"
        return
    fi
    local file_scopes
    file_scopes=$(python3 -c "import sys; sys.path.insert(0, '${REPO_ROOT}'); from config.local_identity import fanout_owner_scopes; print('\n'.join(fanout_owner_scopes()))" 2>/dev/null || true)
...
build_parent_owner_condition() {
    local condition=""
    local owner
    for owner in "${OWNERS[@]}"; do
        local clause=".parent.owner.login == \"${owner}\""
        if [[ -z "${condition}" ]]; then
            condition="${clause}"
        else
            condition="${condition} or ${clause}"
        fi
    done
    printf '%s' "${condition}"
}
...
enumerate_target_repos() {
    local all_repos=()
    local parent_owner_condition
    parent_owner_condition=$(build_parent_owner_condition)
    for owner in "${OWNERS[@]}"; do
...
main() {
    mapfile -t OWNERS < <(resolve_owner_scopes)
    while [[ $# -gt 0 ]]; do
        case "$1" in
