...
| `fan_out_conclusion_report.py` | Post-dispatch reporter for the AI rules fan-out. Re-enumerates the same target repos and logs one line per repo with this dispatch's listener conclusion (`succeeded`, `failed`, `pending`, `listener-missing`, `no-matching-run`, or `opted-out`), correlating a listener run by its `created_at` against the `DISPATCHED_AT` floor. Redacts private or unknown-visibility target names to an owner scope plus a sha256 prefix. Run by `.github/workflows/fan-out-ai-rules.yml` after the dispatch step so a failed listener names its repo. |
...
```bash
FANOUT_OWNER_SCOPES=example-owner EXAMPLEOWNER_TOKEN=<token> python scripts/fan_out_dispatch.py
```
